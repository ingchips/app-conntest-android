...
class BLEUtil private constructor(private val context: Context) {
    ...
    private val chConnStateEvents0: BlockingDeque<ConnectionStateChangeEvent> = LinkedBlockingDeque(10)
    private val gattCallback0: BluetoothGattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {

            Log.i("Press hello", "onConnectionStateChange: $status,$newState")
            ...
            chConnStateEvents0.putFirst(ConnectionStateChangeEvent(gatt, status, newState))
        }
    }
    ...
    fun startScanSpecifiedAddress0(address: String, callback: OnScanResult, durationMilli: Long) {
        if (!checkPermission()) return
        if (scanCallback != null) {
            stopScan()
        }
    ...
    data class ConnectResult (var gatt: BluetoothGatt?, var isOk: Boolean, var status: Int, var newState: Int)
    data class DisconnectResult (var disconnected: Boolean, var status: Int, var newState: Int)

    companion object {
        private val TAG: String = BLEUtil.javaClass.simpleName
        ...
        @JvmStatic
        fun startScanSpecifiedAddress(address: String, callback: OnScanResult, durationMilli: Long) {
            if (!isReady) return
            instance!!.startScanSpecifiedAddress0(address, callback, durationMilli)
        }
        ...
        @JvmStatic
        fun connect0(device: BluetoothDevice): ConnectResult? {
            if (!isReady) return ConnectResult(null,false, 0, 0)
            if (!instance!!.checkPermission()) return ConnectResult(null,false, 0, 0)

            val gatt: BluetoothGatt = device.connectGatt(
                instance!!.context,
                false,
                instance!!.gattCallback0
            )

            val r = instance!!.chConnStateEvents0.takeLast()
            return ConnectResult(gatt,
                r.newState == BluetoothGatt.STATE_CONNECTED, r.status, r.newState)
        }
        @JvmStatic
        fun disconnect0(gatt: BluetoothGatt): DisconnectResult? {
            if (!isReady) return null
            if (!instance!!.checkPermission()) return null
            ...
            var r = instance!!.chConnStateEvents0.takeLast()


            Log.i("hello", "disconnect0: " )
            return DisconnectResult(
                r.newState == BluetoothGatt.STATE_DISCONNECTED, r.status, r.newState)
        }
